...
- June 30, 2025: Fixed node ID extraction from real Meshtastic devices - replaced fake USB-based node ID generation (like "2886_59_mcij12sx") with authentic protobuf extraction of real device node IDs (like "!da75d1c4"), system now properly reads and updates with actual device-provided node IDs from protobuf MyNodeInfo responses, ensuring accurate node identification for SAR operations
- June 30, 2025: Implemented automatic configuration request system - added immediate "wantConfig" request after serial connection to trigger device configuration response (mimicking phone app behavior), enhanced byte-to-text debugging with HEX/ASCII/BINARY displays, added console output detection and API mode switching capability, resolved device communication mode issues where devices send debug text instead of protobuf data
- June 30, 2025: Fixed fragmented data processing in web serial - implemented proper console text buffer accumulation, real-time node ID extraction from fragmented console debug output, enhanced data processing logic to handle both console and binary modes, added robust buffer management for incomplete packets, resolved issue where device responds correctly but fragments were not properly assembled for node ID extraction
