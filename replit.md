...
- June 28, 2025: Integrated official Meshtastic web client features - completely rebuilt Nodes Control Mode page to mirror the official Meshtastic web client at https://github.com/meshtastic/web, added Web Serial API support for direct USB device connectivity in Chrome/Edge browsers, implemented comprehensive node table with filtering/sorting capabilities, hardware model recognition, signal strength visualization, detailed node information expansion, and dual connectivity options (Web Serial + bridge) for maximum compatibility
- June 29, 2025: Fixed critical frontend-backend data flow issue - resolved React Query infinite caching problem that prevented nodes from displaying despite successful database storage, changed staleTime from Infinity to 0 to allow fresh data fetching, verified backend correctly returns 3 test nodes with complete telemetry data
- June 29, 2025: Added "Update All Metadata" functionality - implemented comprehensive metadata refresh system for all nodes with backend API endpoint, simulates querying each node for fresh telemetry data, reports responsive/unresponsive nodes to user, automatically updates database with latest battery/signal/location data, includes role-based access control for admin/user roles
- June 29, 2025: Completely removed all node-related functionality - systematically eliminated node control mode, node API routes, node database storage methods, node UI components, and node status tracking from entire application, Dashboard now focuses exclusively on drone control, messaging, and tactical mapping without any Meshtastic node references
