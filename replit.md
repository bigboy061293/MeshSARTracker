...
- June 28, 2025: Fixed UAS Control Mode data display issues - resolved React Query cache problems with aggressive refresh settings (1Hz), corrected schema field name mismatches, cleared duplicate drone database entries, enhanced MAVLink service to use actual system ID from heartbeat messages for drone identification, implemented proper connection status tracking to prevent fake drone creation without live telemetry data
